...
# Generated by the command: plugin --type source --dir /home/mirolab/src/aruco_plugin --install-dir /usr/local aruco
# Hostname: raspberrypi
# Current working directory: /home/mirolab/src
# Creation date: 2025-09-23T09:02:16.774+0100
...
public:
  ...
  void acquireFrames() {
    ...
    _color_frame = std::make_unique<rs2::video_frame>(_frames->get_color_frame());
    _depth_frame = std::make_unique<rs2::depth_frame>(_frames->get_depth_frame());

    _color_img = cv::Mat(cv::Size(640, 480), CV_8UC3, (void*)_color_frame->get_data(), cv::Mat::AUTO_STEP);
  }
  ...
  void estimatePose() {
      ...
      rs2::video_stream_profile color_profile = _color_frame->get_profile().as<rs2::video_stream_profile>();
      rs2_intrinsics intrinsics = color_profile.get_intrinsics();
      for (size_t i = 0; i < _ids.size(); ++i) {
          cv::Point2f center(0,0);
          for (int j = 0; j < 4; ++j) center += _corners[i][j];
          ...
          _arucos_data.push_back(arucoStruct{_ids[i], point[0], point[1], point[2]});
        }
    }
  ...
  return_type get_output(json &out,
    ...
    //if (!_agent_id.empty()) out["agent_id"] = _agent_id;
     ...
     for (const auto& m : _arucos_data) {
        out["tag." + to_string(m.id)] = {{"position", {m.x, m.y, m.z}}};
        // out = {{"id", m.id}, {"position", {m.x, m.y, m.z}}};
      }
...
private:
    ...
    std::vector<arucoStruct> _arucos_data;
